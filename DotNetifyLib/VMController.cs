...
namespace DotNetify
{
    ...
    public class VMController : IDisposable
    {
        ...
        public delegate void FilterDelegate(string vmId, IBaseVM vm, object data, Action<object> vmAction);
        ...
        protected internal class VMInfo
        {
            ...
            /// </summary>
            public IBaseVM Instance { get; set; }
        ...
        public VMController()
        {
            RequestVMFilter = (string vmId, IBaseVM vm, object vmArg, Action<object> vmAction) => vmAction(vmArg);
            UpdateVMFilter = (string vmId, IBaseVM vm, object vmData, Action<object> vmAction) => vmAction(vmData);
            ResponseVMFilter = (string vmId, IBaseVM vm, object vmData, Action<object> vmAction) => vmAction(vmData);
        }
        ...
        public virtual void Dispose()
        {
            ...
            {
                if (kvp.Value.Instance is IPushVM pushVM)
                    (kvp.Value.Instance as IPushVM).RequestPushUpdates -= VmInstance_RequestPushUpdates;

                if (kvp.Value.Instance is IDisposable)
                    (kvp.Value.Instance as IDisposable).Dispose();
            }
        }
        ...
        public static void RegisterAssembly(Assembly vmAssembly)
        {
            ...
            foreach (Type vmType in vmAssembly.GetExportedTypes().Where(i => typeof(IBaseVM).GetTypeInfo().IsAssignableFrom(i)))
            {
                hasVMTypes = true;
        ...
        public virtual void OnRequestVM(string connectionId, string vmId, object vmArg = null)
        {
            ...
            IBaseVM vmInstance = !_activeVMs.ContainsKey(vmId) ? CreateVM(vmId, vmArg) : _activeVMs[vmId].Instance;
             ...
             if (!_activeVMs.ContainsKey(vmId))
                {
                    ...
                    if (vmInstance is IPushVM)
                        (vmInstance as IPushVM).RequestPushUpdates += VmInstance_RequestPushUpdates;
                }
                else
                ...
                foreach (var id in vmIds.OrderByDescending(i => i.Length))
                {
                    ...
                    if (_activeVMs.TryRemove(id, out vmInfo))
                    {
                        ...
                        if (id.Contains('.'))
                        {
                            ...
                            {
                                if (_activeVMs[masterVMId].Instance is IMasterViewVM)
                                    (_activeVMs[masterVMId].Instance as IMasterViewVM).OnSubVMDisposing(vmInfo.Instance);
                            }
                                
                        }

                        if (vmInfo.Instance is IPushVM)
                            (vmInfo.Instance as IPushVM).RequestPushUpdates -= VmInstance_RequestPushUpdates;
                        if (vmInfo.Instance is IDisposable)
                            (vmInfo.Instance as IDisposable).Dispose();
                    }
                }
            }
        }
        ...
        protected virtual IBaseVM CreateVM(string vmId, object vmArg = null, string vmNamespace = null)
        {
            ...
            var vmInstance = (masterVM is IMasterViewVM masterView) ? masterView?.GetSubVM(vmTypeName, vmInstanceId) : null;
            ...
            if (vmInstance == null)
            {
                ...
                try
                {
                    if (vmInstanceId != null)
                        vmInstance = CreateInstance(vmType, new object[] { vmInstanceId }) as IBaseVM;
                }
                ...
                {
                    if (vmInstance == null)
                        vmInstance = CreateInstance(vmType, null) as IBaseVM;
                }
                catch (MissingMethodException)
                {
            ...
            if (masterVM is IMasterViewVM)
                (masterVM as IMasterViewVM)?.OnSubVMCreated(vmInstance);
        ...
        protected virtual void UpdateVM(IBaseVM vmInstance, string vmPath, string newValue)
        {
            try
            {
